...
if(isset($_GET["search"])) {
  ...
  $query = mysqli_query($con, "SELECT id, firstname, fnumber, snumber, shopaddress, note, date FROM shop_info WHERE date BETWEEN '$fromDate' AND '$toDate'");
  $row = mysqli_fetch_assoc($query);
} else {
  $query = mysqli_query($con, "SELECT id, firstname, fnumber, snumber, shopaddress, note, date FROM shop_info ORDER BY shop_info.id DESC");
  $row = mysqli_fetch_assoc($query);
}
?>
...
<head>
  ...
  <title>سیستم مدیریت فروشگاه / فهرست دکاکین</title>
   ...
   <section class="content container-fluid">
    <div class="row" style="text-align: center;">
        <div class="col-sm-12">
            <div class="titlebody">
               <div class="title">
                 <span id="title">فهرست دکاکین</span>
               </div>
            </div>
        </div>
    </div>
    ...
    <div class="box box-info" id="box">
      ...
      <div class="box-header">
            <div class="row" id="search">
              ...
              <div class="col-md-4">
                  <label for="to-date-man">نا ناریخ</label>
              ...
              <div class="btn-group print" style="float:right; margin-left: 20px;" id="operations">
                  ...
                  <a href="Shop_Info_Record.php" class="btn btn-primary" title="ایجاد دکان جدید"><span class="fa fa-credit-card"></span></a>
        ...
        <div class="box-body" style="overflow: hidden; overflow-x: auto;">
          <table id="shop-table" class="table table-bordered table-striped col-sm-12" style="margin-bottom: 0px !important;">
            <thead>
              <tr>
                <th>آی دی</th>
                <th>نام</th>
...
<script>
    function ExportToExcel(type, fn, dl) {
      //get a copy of table
     var elt = document.getElementById('shop-table').cloneNode(true);
      // remove table header last cell
     elt.children[0].children[0].children[7].remove();
     //remove table body last cells
     let len = elt.rows.length - 1;
     for(let i = 0 ; i<len ; i++){
         elt.children[1].children[i].children[7].remove();
     }
     var wb = XLSX.utils.table_to_book(elt, { sheet: "sheet1" });
     return dl ?
       XLSX.write(wb, { bookType: type, bookSST: true, type: 'base64' }):
       XLSX.writeFile(wb, fn || ('دکاکین_ثبت_شده.' + (type || 'xlsx')));
  }
